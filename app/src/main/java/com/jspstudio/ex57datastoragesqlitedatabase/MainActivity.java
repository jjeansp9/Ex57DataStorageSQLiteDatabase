package com.jspstudio.ex57datastoragesqlitedatabase;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.btn_select_all).setOnClickListener(view -> clickSelectAll());
        findViewById(R.id.btn_select_by_name).setOnClickListener(view -> clickSelectByName());
        ...
        // SQL 언어를 이용해서 원하는 명령을 Database에 실행
    ...
    void clickUpdate(){
        // 해당하는 이름을 가진 데이터의 age를 40으로, email은 "kk@kk.com"으로 변경해보기
        String name= etName.getText().toString();
    ...
    void clickSelectAll(){
        // "member" 테이블의 모든 레코드(한줄 - row)를 가져오기

        // SELECT 검색조건에 따른 결과표를 가리키는 cursor 객체가 리턴됨
        Cursor cursor= db.rawQuery("SELECT * FROM member", null);
        if(cursor == null) return;

        // 총 레코드 수(데이터의 행(row) 개수)
